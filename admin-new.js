...
window.openLeadModal = (leadId) => {
    ...
    if (docs && docs.length > 0) {
        docs.forEach(doc => {
            ...
            // We need a signed URL or public URL logic here. For now assuming public bucket or just showing name.
            // If you have a download function, link it here.
            
            // To properly link, we'd need: supabase.storage.from('kyc-documents').getPublicUrl(doc.path)
            // Let's try to generate a link if path exists
            let fileUrl = '#'
            if (doc.path) {
